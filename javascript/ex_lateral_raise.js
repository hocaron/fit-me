...
document.getElementById("image").innerHTML="<img src='./data/lateralraise/lateralraise.gif'>";



function setup() {
  document.getElementById("method").innerHTML='레터럴 레이즈 하는 방법';


  var timer = setInterval(function(){
    if (timerValue==0){
      loop();
        document.getElementById("method").innerHTML='발을 어깨넓이만큼 벌리고, 팔을 어깨높이까지 들어올리세요';

      document.getElementById("image").innerHTML="<img src='./data/lateralraise/lateralraise1.jpg'>";
      clearInterval(timer)
    }   timerValue--;
  },1000);
...
function gotPoses(poses) {
  ...
  if (r_arm<100&r_arm>70&l_arm<100&l_arm>70){
    poseLabel='raise'; count_set.add(poseLabel);
  } 
  else if (r_arm>-10 & r_arm<20 & l_arm>-10&l_arm<20){
...
if(parseInt(count/2)==5){
  ding.play()
  poseLabel = "Clear";
  location.href="./ex_goodmorning.html"
  }
...
function draw() {
  ...
  // text('오른 팔 각도:'+parseInt(r_arm)+ '\n왼 팔 각도 :' +parseInt(l_arm),width/2, 480);
